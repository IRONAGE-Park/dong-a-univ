...
int safeCheck() { // 안전상태 체크
	...
	for (int k = 0; k < n; k++) {
		for (int i = 0; i < n; i++) {
			if (check[i] == 0) {
				for (int j = 0; j < m; j++) {
					if (Allocation[i][j] + Available[j] >= MAX[i][j]) { // MAX값 이상일 경우에는 안전
						if (j == m - 1) {
							check[i] = 1;
						}
					}
					else break; // 그렇지않으면 보류
				}
			}
			else continue;
		}
	...
	for (int i = 0; i < n; i++) {
		if (check[i] == 1) break;
		else if (check[i] == 0 && i == n - 1) return 0; // 모두 안전하지 않으면 
	}
	return 1;
}
...
int main() {
	...
	for (int i = 0; i < n; i++) for (int j = 0; j < m; j++) fscanf(inp, "%d", &MAX[i][j]); // 요구하는 갯수
